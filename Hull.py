#  File: Hull.py

#  Description: Use Graham's scan algorithm to solve the convex hull problem. This will be in order O(n log n). Also, find the area of the convex hull.

#  Student Name: Raul Denicolas

#  Course Name: CS 313E

#  Unique Number: 52038

#  Date Created: 2/17/2023

#  Date Last Modified: 2/20/2023

import sys
import math

class Point (object):
  # constructor
  def __init__(self, x = 0, y = 0):
    self.x = x
    self.y = y

  # get the distance to another Point object
  def dist (self, other):
    return math.hypot (self.x - other.x, self.y - other.y)

  # string representation of a Point
  def __str__ (self):
    return '(' + str(self.x) + ', ' + str(self.y) + ')'

  # equality tests of two Points
  def __eq__ (self, other):
    tol = 1.0e-8
    return ((abs(self.x - other.x) < tol) and (abs(self.y - other.y) < tol))

  def __ne__ (self, other):
    tol = 1.0e-8
    return ((abs(self.x - other.x) >= tol) or (abs(self.y - other.y) >= tol))

  def __lt__ (self, other):
    tol = 1.0e-8
    if (abs(self.x - other.x) < tol):
      if (abs(self.y - other.y) < tol):
        return False
      else:
        return (self.y < other.y)
    return (self.x < other.x)

  def __le__ (self, other):
    tol = 1.0e-8
    if (abs(self.x - other.x) < tol):
      if (abs(self.y - other.y) < tol):
        return True
      else:
        return (self.y <= other.y)
    return (self.x <= other.x)

  def __gt__ (self, other):
    tol = 1.0e-8
    if (abs(self.x - other.x) < tol):
      if (abs(self.y - other.y) < tol):
        return False
      else:
        return (self.y > other.y)
    return (self.x > other.x)

  def __ge__ (self, other):
    tol = 1.0e-8
    if (abs(self.x - other.x) < tol):
      if (abs(self.y - other.y) < tol):
        return True
      else:
        return (self.y >= other.y)
    return (self.x >= other.x)

# Input: p, q, r are Point objects
# Output: compute the determinant and return the value
def det (p, q, r):
  determinant = (q.x * r.y - q.y * r.x) - p.x * (r.y - q.y) + p.y * (r.x - q.x)
  #det = (x1 * y2 + x2 * y3 + ... + xn * y1 - y1 * x2 - y2 * x3 - ... - yn * x1)
  return determinant

# Input: sorted_points is a sorted list of Point objects
# Output: computes the convex hull of a sorted list of Point objects
#         convex hull is a list of Point objects starting at the
#         extreme left point and going clockwise in order
#         returns the convex hull
def convex_hull (sorted_points):
  n = len(sorted_points)
  upper_hull = []
  upper_hull.append(sorted_points[0])
  upper_hull.append(sorted_points[1])
  for i in range(2, n):
    #5. append p_i to upper_hull
    upper_hull.append(sorted_points[i])
    while len(upper_hull) >= 3 and not (det(upper_hull[-1], upper_hull[-2], upper_hull[-3]) > 0):
      del upper_hull[-2] 
  lower_hull = []
  lower_hull.append(sorted_points[-1])
  lower_hull.append(sorted_points[-2])
  for j in range(n - 3, -1, -1):
    lower_hull.append(sorted_points[j])
    while len(lower_hull) >= 3 and not det(lower_hull[-1], lower_hull[-2], lower_hull[-3]) > 0:
      del lower_hull[-2]
  del lower_hull[0]
  del lower_hull[-1]
  for point in lower_hull:
    upper_hull.append(point)
  return upper_hull

# Input: convex_poly is  a list of Point objects that define the
#        vertices of a convex polygon in order
# Output: computes and returns the area of a convex polygon
def area_poly (convex_poly):
  determinant = 0
  for i in range(len(convex_poly)):
    if i == len(convex_poly) - 1:
      determinant += convex_poly[i].x * convex_poly[0].y
    else:
      determinant += convex_poly[i].x * convex_poly[i + 1].y
  for j in range(len(convex_poly)):
    if j == len(convex_poly) - 1:
      determinant -= convex_poly[j].y * convex_poly[0].x
    else:
      determinant -= convex_poly[j].y * convex_poly[j + 1].x
  area = (1 / 2) * abs(determinant)
  return area

# Input: no input
# Output: a string denoting all test cases have passed
def test_cases():
  # write your own test cases
  return "all test cases passed"

def main():
  # create an empty list of Point objects
  points_list = []
  # read number of points
  line = sys.stdin.readline()
  line = line.strip()
  num_points = int (line)

  # read data from standard input
  for i in range (num_points):
    line = sys.stdin.readline()
    line = line.strip()
    line = line.split()
    x = int (line[0])
    y = int (line[1])
    points_list.append (Point (x, y))
    #print(Point (x, y))
  # sort the list according to x-coordinates
    sorted_points = sorted(points_list)
  # print the sorted list of Point objects
  #for p in sorted_points:
  #  print (str(p))
  # get the convex hull
  convex_hull_list = convex_hull(sorted_points)
  # run your test cases
  # print your results to standard output
  # print the convex hull
  print('Convex Hull')
  for i in convex_hull_list:
    print(str(i))
  #print(convex_hull_list)
  # get the area of the convex hull
    area_convex_hull = area_poly(convex_hull_list)
  print()
  # print the area of the convex hull
  print(f'Area of Convex Hull = {str(area_convex_hull)}')#p, q, r

if __name__ == "__main__":
  main()
