...
#  Description: Find solutions to the following 8 problems using recursions (without the use of loops)
...
#You have a bunch of numbers and you are trying ot see if there is a subset in those numbers that add up to the target
def groupSum(start, nums, target):
   if (start >= len(nums)):
      return (target == 0)
   else:
      return groupSum(start + 1, nums, target - nums[start]) or groupSum(start + 1, nums, target)#I either subtract or I don't subtract
...
def groupSum6(start, nums, target):
   if (start >= len(nums)):
      ...
      return groupSum6(start + 1, nums, target - nums[start]) or groupSum6(start + 1, nums, target) #I either subtract or I don't subtract
...
def groupSum5(start, nums, target):
   if (start >= len(nums)):
      ...
      return groupSum5(start + 1, nums, target - nums[start]) or groupSum5(start + 1, nums, target) #I either subtract or I don't subtract
    ...
    if start >= len(nums):
        ...
        for i in range(len(nums) - start):
            if nums[start+i] == nums[start]:
                ...
                break
        return groupSumClump(start + extent, nums, target - (nums[start]*extent)) or groupSumClump(start + extent, nums, target)
    else:
...
def splitArrayHelper(nums, start, sum1, sum2):
    if (start >= len(nums)):
        return (sum1 == sum2)
    else:
        return (splitArrayHelper(nums, start+1, sum1+nums[start], sum2) or splitArrayHelper(nums, start+1, sum1, sum2+nums[start]))
...
def splitOdd10Helper(nums, start, sum1, sum2):
    if (start >= len(nums)):
        ...
        return (splitOdd10Helper(nums, start+1, sum1+nums[start], sum2) or splitOdd10Helper(nums, start+1, sum1, sum2+nums[start]))
...
def split53Helper(nums, start, sum1, sum2):
    if (start >= len(nums)):
        return (sum1 == sum2)
    elif nums[start] % 5 == 0:
        return split53Helper(nums, start+1, sum1+nums[start], sum2)
    elif nums[start] % 3 == 0:
        return split53Helper(nums, start+1, sum1, sum2+nums[start])
    else:
        return (split53Helper(nums, start+1, sum1+nums[start], sum2) or split53Helper(nums, start+1, sum1, sum2+nums[start]))
